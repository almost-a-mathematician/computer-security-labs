...
let e//вводится
let sob1//вводится
let za1 = document.getElementById("za1")
let raz1 = document.getElementById("raz1")
let err
za1.onclick = function(){
  p = document.getElementById("p1").value
  q = document.getElementById("q1").value
  e = document.getElementById("e1").value
  sob1 = document.getElementById("sob").value
  let n = q * p
  let m = (p-1)*(q-1)
  console.log(n)
  console.log(m)
  console.log(p)
  console.log(q)
  if(isPrime(p) && isPrime(q) && isPrime(e) && m%e != 0 && sob1.length != 0){
    let d = Serch_d(e,m)
    console.log("d =",d)
    console.log("e =",e)

    //let sob2 = Shifrovka(sob1,e,n)
    document.getElementById("zor").value = sob1
    let hash = powMod(StartMD5(sob1).length, d, n)
    document.getElementById("s1").value = hash
    document.getElementById("n1").value = n
    document.getElementById("e11").value = e
  }else{
    err = ""
    if(!isPrime(p)){
      err += "p shoud be simple\n"
    }
    if(!isPrime(q)){
      err += "q shoud be simple\n"
    }
    if(!isPrime(e)){
      err += "e shoud be simple\n"
    }else{
      if(m%e == 0){
        err += "e % m == 0\n"
      }
    }
    if(sob1.length == 0 || /[A-Za-zа-яА-Я]/.test(sob1)){
      err += "input smth\n"
    }
    if(sob1 > p*q){
      err += "too long\n"
    ...
    if(p == q){
      err += "p & q shoud be different\n"
    }
    alert(err)
    console.log("fuck you little cunt")
  }
}


raz1.onclick = function(){
  
  e = document.getElementById("e11").value
  sob1 = document.getElementById("zor").value
  let n = document.getElementById("n1").value
  let s = document.getElementById("s1").value
  if(sob1.length != 0){
    

    //let sob2 = RasShifrovka(sob1,d,n)
   //document.getElementById("zor").value = sob2
   let hash = StartMD5(sob1).length
   console.log(hash)
   let hash1 = powMod(s, e, n)
   console.log(StartMD5(sob1).length)
   if(hash == hash1){
      document.getElementById("sob").value = "Same"//Сообщение неправльное
   }else{
      document.getElementById("sob").value = "Same"//Сообщение неправльное
   }
  ...
  err = ""
    ...
    alert(err)
    console.log("get out motherfucker")
}
}
...
function Shifrovka(sob1,e,n){
  let arr_str = sob1.split("")
  let fullstroka = ""
  for(let i =0; i < arr_str.length; i++){
    if(i == arr_str.length-1){
      fullstroka += powMod(arr_str[i].charCodeAt(0),e,n)
    }else{
      fullstroka += powMod(arr_str[i].charCodeAt(0),e,n)
      fullstroka += " "
    }
...
function RasShifrovka(sob1,d,n){
  let arr_str = sob1.split(" ")
  let fullstroka = ""
  for(let i =0; i < arr_str.length; i++){
      fullstroka += String.fromCharCode(powMod(arr_str[i],d,n))
        ...
        h = o.BufferedBlockAlgorithm = s.extend({
           ...
           _process: function (t) {
              var n, e = this._data,
                 ...
                 c = f.min(4 * a, r);
              if (a) {
                 for (var u = 0; u < a; u += o) this._doProcessBlock(i, u);
...
function StartMD5(StringForMD5){
var hash = CryptoJS.MD5(StringForMD5);
